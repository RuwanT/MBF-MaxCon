...
params_fh = struct('method','full-hypercube', 'degree', 0:2);
params_us = struct('method','uniform-sampling', 'degree', 0:2, 'nsamples', 5000);
...
hold on;
for k=1:nbits
    text(Data(k, 1),Data(k, 3),num2str(k))
end
...
for i = 1:length(method_params)
    ...
    scatter(Data(outls,1), Data(outls,3), outl_s(i), outl_disp{i}); hold on;
    ...
    similarity_mat = get_similarity_mat(fourier_coeff_, nbits);
    subplot(1,3,3)
    % imshow(similarity_mat)
    image(similarity_mat,'CDataMapping','scaled')
    colorbar
